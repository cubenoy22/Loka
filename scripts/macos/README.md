...
- `scripts/macos/gen-xcodeproj-leopard-ub1.sh`
  ...
  - For Tiger 32-bit UB1 builds in Xcode 3.2.6, set the deployment target to Mac OS X 10.4, use only 32-bit architectures, and choose 32-bit Intel plus PPC; verified outputs use `ppc i386` slices. Tiger runtime compatibility does not imply Tiger/Xcode 2.4 project debugging: the Cocoa backend uses Objective-C
`@property` / `@synthesize` in a few internal delegate classes, so use Snow
Leopard/Xcode 3.2.6 for debugging Tiger-targeted builds.
