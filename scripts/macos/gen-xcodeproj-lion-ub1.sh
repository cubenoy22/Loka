...
BUILD_DIR="${BUILD_DIR:-${ROOT_DIR}/build/macos-xcodeproj-lion-ub1/${BUILD_TYPE}-${BUILD_VARIANT}}"
...
echo "[gen-xcodeproj-lion-ub1] ROOT_DIR=${ROOT_DIR}"
echo "[gen-xcodeproj-lion-ub1] BUILD_DIR=${BUILD_DIR}"
echo "[gen-xcodeproj-lion-ub1] SDK=${OSX_SYSROOT}"
echo "[gen-xcodeproj-lion-ub1] ARCHS=${ARCHS} DEPLOYMENT_TARGET=${DEPLOYMENT_TARGET}"
if [[ -n "${XCODE_ARCHS}" ]]; then
  echo "[gen-xcodeproj-lion-ub1] XCODE_ARCHS=${XCODE_ARCHS}"
fi
if [[ -n "${XCODE_VALID_ARCHS}" ]]; then
  echo "[gen-xcodeproj-lion-ub1] XCODE_VALID_ARCHS=${XCODE_VALID_ARCHS}"
fi
echo "[gen-xcodeproj-lion-ub1] running: cmake ${CMAKE_ARGS[*]}"
cmake "${CMAKE_ARGS[@]}"

echo "Generated Lion UB1 Xcode project in:"
echo "  ${BUILD_DIR}"
