...
- Before opening, committing, or reviewing a non-trivial change, run a lightweight complexity gate. Count risk flags rather than trying to compute a perfect metric: new owner/lifecycle/cleanup paths; multiple new member fields or mutable flags; changes spanning two or more of State/Flow/Boundary/Platform; new cross-boundary state/callback/pointer sharing; raw callback/thunk lifetime that is not visible from the owner type; dirty/layout/attach/detach/teardown behavior changes; ambiguous public API names; new fallback/default/implicit behavior; platform-specific workarounds leaking into common/app-facing code; or lifecycle/state-routing/layout/dirty behavior changes without tests.
