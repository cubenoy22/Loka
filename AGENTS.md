...
- Lifecycle/update-loop smells to flag explicitly (see docs/case-studies/case-study-scene-update-cycle.md for real instances): a scheduler flag left true while its payload was cleared (scheduled-but-empty split brain); a single `clear()`/reset whose responsibility spans work that belongs to the next cycle; appending to a container an in-flight cursor is still walking; a numeric stamp or phase flag standing in for a structural current/next or phase boundary.
- When a test pins known-bad behavior, verify the loss/failure mechanism by instrumentation before writing the pin comment; do not infer it. A mis-attributed pin comment survives review and misleads the eventual fix (this happened: the apply-window write-loss pin blamed the transaction clear while the write was actually swallowed a layer lower).
- For lifecycle/update-loop changes, combine both review modes deliberately: adversarial micro-reading of the diff (bots are good at this) and cross-layer instrumented verification of the claimed mechanism (run it, print it, prove it). Each catches a class the other misses.
...
- Deliver design work as a rally, not a completed campaign: one topic per exchange, legible without reading code, roughly one page, with a shape diagram (boxes and solid lines) and a concrete input -> expected -> actual example. Oversized work fails review even when it is correct, because the maintainer must be able to stay the deciding party.
- Explain bugs and trade-offs through concrete cases first ("pressing X leaves label Y stale") and unit-level input/output contracts; keep abstract mechanism descriptions as the follow-up, not the lead.
- One failure mode = one issue = one test = one fix. Name issues by symptom ("[Bug] doing X leaves Y in state Z"), keep a reproduction fixture per failure mode, and state in the issue what is guaranteed once it closes. Discovering a second loss point mid-fix means filing a second issue, not widening the first.
- Merging to the default branch is always the maintainer's action. Known-hole merges (a fix that lands while a related documented hole remains) require an explicit per-case agreement recorded on the PR, never a silent "next PR will fix it".
