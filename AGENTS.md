...
- Lifecycle/update-loop smells to flag explicitly (see docs/case-study-scene-update-cycle.md for real instances): a scheduler flag left true while its payload was cleared (scheduled-but-empty split brain); a single `clear()`/reset whose responsibility spans work that belongs to the next cycle; appending to a container an in-flight cursor is still walking; a numeric stamp or phase flag standing in for a structural current/next or phase boundary.
