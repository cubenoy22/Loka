...
namespace smirkbench
{
  ...
  public:
    ...
    bool addFace()
    {
      if (this->faceCount_ >= loka::app::RectSurfaceModel::kMaxSprites)
      {
        return false;
      }
    ...
    {
      return this->faceCount_ < loka::app::RectSurfaceModel::kMaxSprites;
    }
  ...
  private:
    ...
    void publishSurface()
    {
      ...
      for (short i = 0; i < this->faceCount_; ++i)
      {
        // The face count is capped at kMaxSprites, so add() cannot refuse here.
        const bool added = this->cachedModel_.add(this->faces_[i].sprite);
        assert(added);
        (void)added;
      }
    ...
    Face faces_[loka::app::RectSurfaceModel::kMaxSprites];
    short faceCount_;
    short boundsWidth_;
    short boundsHeight_;
