...
void testDynamicBoundaryLocalDiffReleasesRetiredSubtrees()
{
  ...
  {
  public:
    CleanupCapturePlatformController() : releaseCalls_(0), lastReleased_(0), lastReleasedTag_(0) {}
    ...
    virtual void releaseNodeContexts(Node *node)
    {
      ++releaseCalls_;
      lastReleased_ = node;
      lastReleasedTag_ = node ? node->nodeTag() : 0;
    }

    int releaseCalls_;
    Node *lastReleased_;
    NodeTag lastReleasedTag_;
  };

  class DynamicLocalCleanupProbeNode : public DynamicCompositionBoundaryNodeBase<BoundaryPropsFor<DynamicLocalCleanupProbeNode> >
  {
  public:
...
void testFrozenBoundaryIgnoresObservedStateInvalidation()
{
  ...
  using loka::app::scene::BoundaryNodeFor;
  ...
  using loka::app::scene::ObservedStateRegistrar;
  ...
  class FrozenObservedDynamicNode;
  struct FrozenObservedDynamicTypeTag {};
  struct FrozenObservedDynamicProps : public loka::app::scene::NodePropsBase<FrozenObservedDynamicProps>
  {
    typedef FrozenObservedDynamicTypeTag TypeTag;
    typedef FrozenObservedDynamicNode NodeType;

    loka::core::State<bool> *observed_;

    FrozenObservedDynamicProps() : observed_(0) {}
    ...
    bool operator<(const loka::app::scene::PropsBase &rhs) const
    {
      ...
      const FrozenObservedDynamicProps &other = static_cast<const FrozenObservedDynamicProps &>(rhs);
      return this->observed_ < other.observed_;
    }
  };

  class FrozenObservedDynamicNode : public DynamicCompositionBoundaryNodeBase<FrozenObservedDynamicProps>
  {
  public:
    FrozenObservedDynamicNode(const FrozenObservedDynamicProps &p)
        : DynamicCompositionBoundaryNodeBase<FrozenObservedDynamicProps>(p) {}

    virtual void composeNode(loka::app::scene::NodeComposition &c)
    {
      ++g_frozenObservedComposeCount;
      c.declare(this->props.observed_ && this->props.observed_->get()
                    ? loka::app::Text("Frozen On").tag(20)
                    : loka::app::Text("Frozen Off").tag(20));
    }

    virtual void declareObservedStates(ObservedStateRegistrar &registrar)
    {
      if (this->props.observed_)
      {
        registrar.observe(this->props.observed_, loka::app::scene::NODE_DIRTY_CHILD);
      }
    }
  };

  class FrozenObservedHostNode : public BoundaryNodeFor<FrozenObservedHostNode>
  {
  public:
    FrozenObservedHostNode(const BoundaryPropsFor<FrozenObservedHostNode> &p)
        : BoundaryNodeFor<FrozenObservedHostNode>(p),
          observed_()
    {
    }
    ...
    virtual void attachNode(loka::app::scene::NodeComposition &c)
    {
      ...
      g_frozenObservedState = static_cast<loka::core::MutableState<bool> *>(this->observed_.state());
    }

    virtual void composeNode(loka::app::scene::NodeComposition &c)
    {
      FrozenObservedDynamicProps props;
      props.observed_ = this->observed_.state();
      c.declare(BoundaryDefinition<FrozenObservedDynamicProps, FrozenObservedDynamicNode>(props).tag(10));
    }
  ...
  g_frozenObservedComposeCount = 0;
  g_frozenObservedState = 0;

  Scene scene((BoundaryDefinition<BoundaryPropsFor<FrozenObservedHostNode>, FrozenObservedHostNode>()));
  ...
  BoundaryNode *dynamicBoundary = rootBoundary->childrenHead() ? rootBoundary->childrenHead()->asBoundary() : 0;
  assert(dynamicBoundary != 0);

  dynamicBoundary->setFrozen(true);
  {
...
namespace
{
  ...
  static loka::app::scene::Node *findNodeByTagRecursive(loka::app::scene::Node *node, loka::app::scene::NodeTag tag)
  {
    if (!node)
    {
      return 0;
    ...
    while (child)
    {
      loka::app::scene::Node *found = findNodeByTagRecursive(child, tag);
      if (found)
      {
        return found;
      }
...
void testSceneMixedStaticAndDynamicChildDirtyResolvesToPropsOnly()
{
  ...
  public:
    ...
    void toggleDetailsAndRefresh()
    {
      this->detailsVisible_.set(!this->detailsVisible_.get(), true);
      this->statusText_.set(this->detailsVisible_.get()
                                ? loka::core::String::Literal("Details: shown")
                                : loka::core::String::Literal("Details: hidden"),
                            true);
    }
...
void testSceneMixedStaticAndDynamicChildDirtyTracksBoundaryLocalDiffState()
{
  ...
  Node *rootNode = SceneTestAccess::rootNode(scene);
  assert(rootNode != 0);
  BoundaryNode *staticBoundary = findNodeByTagRecursive(rootNode, 110) ? findNodeByTagRecursive(rootNode, 110)->asBoundary() : 0;
  BoundaryNode *dynamicBoundary = findNodeByTagRecursive(rootNode, 120) ? findNodeByTagRecursive(rootNode, 120)->asBoundary() : 0;
  assert(staticBoundary != 0);
  assert(dynamicBoundary != 0);

  assert(staticBoundary->hasLocalCompositionDiff() == false);
...
void testSceneMixedDynamicRootChildDirtyDowngradesFullRebuild()
{
  ...
  class MixedDynamicRootNode : public DynamicCompositionBoundaryNodeBase<BoundaryPropsFor<MixedDynamicRootNode> >
  {
  public:
    typedef BoundaryPropsFor<MixedDynamicRootNode> PropsType;

    MixedDynamicRootNode(const PropsType &p)
        : DynamicCompositionBoundaryNodeBase<PropsType>(PropsType(p)),
          initialized_(false),
          detailsVisible_(),
          statusText_() {}
  ...
  MixedDynamicRootNode *root = static_cast<MixedDynamicRootNode *>(SceneTestAccess::rootBoundary(scene));
  assert(root != 0);
  assert(platform.calls_ == 1);
