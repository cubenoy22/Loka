...
void testSmirkModelReflectsRefusesAndReclamps()
{
  ...
  for (int i = 0; i < loka::app::RectSurfaceModel::kMaxSprites; ++i)
  {
    LOKA_VERIFY(cappedModel.addFace());
  }
  LOKA_VERIFY(!cappedModel.addFace());
  LOKA_VERIFY(cappedModel.faceCount() == loka::app::RectSurfaceModel::kMaxSprites);
  ...
  while (fixture.model.faceCount() < loka::app::RectSurfaceModel::kMaxSprites)
  {
