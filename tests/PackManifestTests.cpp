...
{
  const PackManifest manifest = Manifest("bag ui\n"
  ...
  std::string header;
  LOKA_VERIFY(loka::lrpc::GenerateResourceHeader(manifest, header) == loka::lrpc::RESOURCE_HEADER_OK);
  ...
  assert(header.find("const AssetRef RefusedBadge = {9001UL, 0, ") != std::string::npos);
  assert(header.find("const AssetRef Caption = {9002UL, 1, ") != std::string::npos);
  ...
  assert(header.find("const std::size_t AssetCount = 2;", header.find(uiOpening)) != std::string::npos);

  const PackManifest renamedSources = Manifest("bag ui\n"
  ...
  LOKA_VERIFY(loka::lrpc::GenerateResourceHeader(renamedSources, sameHeader) == loka::lrpc::RESOURCE_HEADER_OK);
  assert(header == sameHeader);

  const PackManifest oneListing = Manifest("bag all\n"
                                           "asset 7 image UI/Icon icon\n"
                                           "asset 3 string Pages/First first\n"
                                           "asset 5 string Pages/Second second\n");
  const PackManifest anotherListing = Manifest("bag all\n"
                                               "asset 5 string Pages/Second second\n"
                                               "asset 7 image UI/Icon icon\n"
                                               "asset 3 string Pages/First first\n");
  std::string canonicalOne;
  std::string canonicalOther;
  LOKA_VERIFY(loka::lrpc::GenerateResourceHeader(oneListing, canonicalOne) == loka::lrpc::RESOURCE_HEADER_OK);
  LOKA_VERIFY(loka::lrpc::GenerateResourceHeader(anotherListing, canonicalOther) == loka::lrpc::RESOURCE_HEADER_OK);
  assert(canonicalOne == canonicalOther);
  assert(canonicalOne.find("const AssetRef First") < canonicalOne.find("const AssetRef Second"));
...
{
  const char *badSymbols[] = {"bag Main\nasset 1 image 9Lives a\n",
  ...
  std::string preserved("preserved");
  LOKA_VERIFY(loka::lrpc::GenerateResourceHeader(Manifest("bag Main\nasset 1 image UI/Badge a\n"
                                                          "asset 2 image UI/Badge/Small b\n"),
                                                 preserved)
              == loka::lrpc::RESOURCE_HEADER_SYMBOL_COLLISION);
  assert(preserved == "preserved");

  LOKA_VERIFY(loka::lrpc::GenerateResourceHeader(Manifest("bag Main\nasset 1 image Pages/Assets a\n"), preserved)
              == loka::lrpc::RESOURCE_HEADER_RESERVED_SYMBOL);
  assert(preserved == "preserved");
...
void testResourceHeaderHandlesFiftyThousandSymbols()
{
  ...
  std::string header;
  LOKA_VERIFY(loka::lrpc::GenerateResourceHeader(manifest, header) == loka::lrpc::RESOURCE_HEADER_OK);
  assert(header.find("const std::size_t AssetCount = 50000;") != std::string::npos);
  assert(header.find("const AssetRef A50000 = {50000UL, 0,") != std::string::npos);
...
void testPackManifestParsesRecordsAndRefusesMalformedLines()
{
  ...
  LOKA_VERIFY(Refusal("bag Main\nasset 1 image N s\nbogus x\n", line) == loka::lrpc::MANIFEST_UNKNOWN_DIRECTIVE);
  assert(line == 3);
  ...
  assert(line == 1);
  LOKA_VERIFY(Refusal("bag Main\nasset 1 image A a\nasset 1 string B b\n", line) == loka::lrpc::MANIFEST_DUPLICATE_ID);
  assert(line == 3);
...
void testPackManifestStampFollowsTheIdSpaceNotTheListing()
{
  ...
  LOKA_VERIFY(Parse("bag Main\nasset 1 image AB a\nasset 2 image C b\n", cutOneWay, line) == loka::lrpc::MANIFEST_OK);
  LOKA_VERIFY(Parse("bag Main\nasset 1 image A a\nasset 2 image BC b\n", cutTheOther, line) == loka::lrpc::MANIFEST_OK);
...
void testPackageRequirementsCheckEveryScrapbookExpectation()
{
  ...
  std::size_t line = 0;
  LOKA_VERIFY(loka::lrpc::CheckPackageRequirements(all, std::strlen(all), satisfied, &pageCount, violations, line)
              == loka::lrpc::REQUIREMENTS_OK);
  assert(violations.empty());

  const PackManifest allWrong = Manifest("bag pages\n"
                                         "asset 1001 image Page/One one\n"
                                         "bag page-1\n"
                                         "bag page-2\n"
                                         "asset 1002 string Page/Two two\n");
  LOKA_VERIFY(loka::lrpc::CheckPackageRequirements(all, std::strlen(all), allWrong, &pageCount, violations, line)
              == loka::lrpc::REQUIREMENTS_OK);
  assert(violations.size() == 4);
  assert(violations[0].line == 1);
  assert(violations[0].message == "bag 0 must be named \"ui\"; found bag 0 named \"pages\"");
  assert(violations[1].line == 2);
  assert(violations[1].message == "asset 9001 must have kind image; found no asset with that id");
  ...
  assert(violations[3].message == "package must contain exactly 3 assets (2 pages plus 1 listed asset); found 2");

  const char *asset = "asset 9001 image in ui\n";
  const PackManifest wrongKind = Manifest("bag ui\n"
                                          "asset 9001 string UI/Badge badge\n");
  LOKA_VERIFY(loka::lrpc::CheckPackageRequirements(asset, std::strlen(asset), wrongKind, 0, violations, line)
              == loka::lrpc::REQUIREMENTS_OK);
  assert(violations.size() == 1);
  assert(violations[0].line == 1);
  assert(violations[0].message == "asset 9001 must have kind image; found kind string");
...
{
  const char *requirements = "bag 0 ui\n"
                             "asset 9001 image in ui\n"
                             "pages 1001 count-from bag 1 kinds image,string\n";
  const PackManifest fourPages = Manifest("bag ui\n"
                                          "asset 9001 image UI/Badge badge\n"
                                          "bag page-1\n"
                                          "asset 1001 image Page/One one\n"
                                          "bag page-2\n"
  ...
  LOKA_VERIFY(loka::lrpc::CheckPackageRequirements(
                  requirements, std::strlen(requirements), fourPages, &pageCount, violations, line)
              == loka::lrpc::REQUIREMENTS_OK);
  ...
  assert(violations[1].message == "package must contain exactly 6 assets (5 pages plus 1 listed asset); found 5");

  LOKA_VERIFY(
      loka::lrpc::CheckPackageRequirements(requirements, std::strlen(requirements), fourPages, 0, violations, line)
      == loka::lrpc::REQUIREMENTS_PAGE_COUNT_REQUIRED);
  assert(line == 3);
...
void testPackageRequirementsRejectExtraAsset()
{
  const char *requirements = "asset 9001 image in ui\n"
                             "pages 1001 count-from bag 1 kinds image,string\n";
  const PackManifest extraAsset = Manifest("bag ui\n"
                                           "asset 9001 image UI/Badge badge\n"
  ...
  const std::size_t pageCount = 2;
  LOKA_VERIFY(loka::lrpc::CheckPackageRequirements(
                  requirements, std::strlen(requirements), extraAsset, &pageCount, violations, line)
              == loka::lrpc::REQUIREMENTS_OK);
  assert(violations.size() == 1);
  assert(violations[0].line == 2);
  assert(violations[0].message == "package must contain exactly 3 assets (2 pages plus 1 listed asset); found 4");
...
void testPackageRequirementsRejectBadPageKind()
{
  const char *requirements = "asset 9001 image in ui\n"
                             "pages 1001 count-from bag 1 kinds image,string\n";
  const PackManifest audioPage = Manifest("bag ui\n"
                                          "asset 9001 image UI/Badge badge\n"
                                          "bag page-1\n"
                                          "asset 1001 image Page/One one\n"
                                          "bag page-2\n"
  ...
  const std::size_t pageCount = 2;
  LOKA_VERIFY(loka::lrpc::CheckPackageRequirements(
                  requirements, std::strlen(requirements), audioPage, &pageCount, violations, line)
              == loka::lrpc::REQUIREMENTS_OK);
  assert(violations.size() == 1);
  assert(violations[0].line == 2);
  assert(violations[0].message == "page asset 1002 must have one of kinds image,string; found kind audio");
...
{
  const char *requirements = "asset 9001 image in ui\n";
  const PackManifest movedBadge = Manifest("bag ui\n"
                                           "bag page-1\n"
                                           "asset 9001 image UI/Badge badge\n");
  std::vector<RequirementViolation> violations;
  std::size_t line = 0;
  LOKA_VERIFY(
      loka::lrpc::CheckPackageRequirements(requirements, std::strlen(requirements), movedBadge, 0, violations, line)
      == loka::lrpc::REQUIREMENTS_OK);
  assert(violations.size() == 1);
  assert(violations[0].line == 1);
  assert(violations[0].message == "asset 9001 must be in bag \"ui\"; found bag 1 (\"page-1\")");
...
void testPackageRequirementsUnreadableFileIsAHardError()
{
  ...
  std::size_t line = 99;
#if defined(_WIN32)
  const RequirementResult result = loka::lrpc::CheckPackageRequirementsFile(L".", manifest, 0, violations, line);
#else
  const RequirementResult result = loka::lrpc::CheckPackageRequirementsFile(".", manifest, 0, violations, line);
#endif
