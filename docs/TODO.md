...
- ProgrammingGuide boundary/state pass: add a dedicated section that explains the boundary-first model clearly: `declareStates()` as the normal owner path, `currentBoundary()` for owner-side access, `findBoundary()` for direct-parent borrowed access, `BoundaryProps` as parent-to-child input, `Managed<T>` as explicit shared access, and why broad cross-boundary state plumbing is intentionally discouraged.
