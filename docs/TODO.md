...
- ImageView scaling contract: `fit` mode handling aligned on Win32/macOS (`NONE/CONTAIN/COVER/STRETCH`); Toolbox draw path still pending, but width-follow + aspect-height auto layout is now aligned across Win32/macOS/Toolbox when `height` is unspecified.
