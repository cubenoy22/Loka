...
- **Node state batch API**: Revisit `declareStates()` as a future Node-local batch registration path for nodes with many `NodeState<T>` members, especially Classic/68k-sensitive code. Keep `this->state(...)` as the simple default for ordinary small state sets.
