...
- **0.0.2 Boundary inner ownership / conditional lifetime policy**: `0.0.1` can ship with the current "works as a first look" behavior, but the next cleanup pass should make subtree-local ownership explicit. Define a `BoundaryInnerStateOwner`-style `IStateOwner` implementation for cases where Boundary lifetime is too broad, replace the ad-hoc `Headless` owner implementation with that shared owner, and decide the default `Show`/conditional behavior. Current `ConditionalNode` retains inactive branches; for map/list-style UIs the likely default should be destroy-on-detach, with any retained/keep-alive policy exposed explicitly rather than hidden.
