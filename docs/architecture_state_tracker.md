# Declara! State / Tracker 実装ノート（2025-11 最新）
...
| `AutoTransactionGuard` | RAII で `begin()`/`end()` を自動管理 | `common/core/AutoTransactionGuard.hpp` |
...
| `EmitterState::emit()` | OS 起点イベント → Declara! の一方向橋渡し。 | Win32Button の `BN_CLICKED` など |
...
- `common/core/AutoTransactionGuard.hpp`
