...
- 明示的に複数の `set()` をまとめたい場合は `loka::core::StateTrackerGuard _(window->getTracker());` を使う。
...
1. **Tracker と Boundary の関係整理**
   - `loka::core::StateTrackerGuard` の運用は Boundary/Window がオーナー、State は Tracker に依存しない前提で統一する。
2. **defer の実行モデル**
