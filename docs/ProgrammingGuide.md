...
低レベルな入口として残っていますが、普通の Node 固有 state では、
...
API として残っています。既存コードや特殊な composition では有効ですが、
...
- 自分の `declareStates()` 由来の `BoundState<T>` を member に持つ: 互換/特殊用途としてよい
...
- 必要な場合だけ `declareStates()` で Boundary composition 側の state batch を扱う
