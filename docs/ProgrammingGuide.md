...
class CounterNode : public loka::app::scene::StaticCompositionNodeFor<CounterNode>
{
public:
  typedef loka::app::scene::StaticCompositionPropsFor<CounterNode> PropsType;

  CounterNode(const PropsType &p)
      : loka::app::scene::StaticCompositionNodeFor<CounterNode>(p),
        count_(),
        countText_(),
        increment_()
  {
...
構造を大きく組み替える前に、
`showIf` のような明示的な切り替えで表現できるかを先に考えると整理しやすくなります。
...
class TogglePanelNode : public loka::app::scene::StaticCompositionNodeFor<TogglePanelNode>
{
public:
  typedef loka::app::scene::StaticCompositionPropsFor<TogglePanelNode> PropsType;

  TogglePanelNode(const PropsType &p)
      : loka::app::scene::StaticCompositionNodeFor<TogglePanelNode>(p),
        detailsVisible_(),
        toggle_(),
...
```cpp
class HelloNode : public loka::app::scene::StaticCompositionNodeFor<HelloNode>
{
public:
  typedef loka::app::scene::StaticCompositionPropsFor<HelloNode> PropsType;

  HelloNode(const PropsType &p)
      : loka::app::scene::StaticCompositionNodeFor<HelloNode>(p)
  {
  }
...
## 12. Static Composition と Dynamic Composition

Loka では、Composition の作り方にも明確な選択肢があります。

- Static Composition
- Dynamic Composition

この違いは、単なる API の差ではなく、
どの程度再構成を許すかという設計判断です。

### Static Composition

Static Composition は、
基本的に一度組み立てた構造を安定的に使う方向の Composition です。
...
まず Static を基準に考えるのはかなり自然です。

### Dynamic Composition

Dynamic Composition は、
状態に応じて構造自体を組み替えたいときに使います。
...
そのぶん、Static よりも設計判断が増えます。
本当に構造を組み替える必要があるのか、
あるいは単なる visible / enabled / text の変更なのかは、
常に一度見極めたほうがよいです。

### まずは Static を基準にする

Loka の実践では、
「まず Static で考え、必要な場所だけ Dynamic にする」
という進め方がかなり重要です。
...
```cpp
class SearchPanelNode : public loka::app::scene::StaticCompositionNodeFor<SearchPanelNode>
{
public:
  typedef loka::app::scene::StaticCompositionPropsFor<SearchPanelNode> PropsType;

  SearchPanelNode(const PropsType &p)
      : loka::app::scene::StaticCompositionNodeFor<SearchPanelNode>(p),
        query_()
  {
  }
...
```cpp
class PanelNode : public loka::app::scene::StaticCompositionNodeFor<PanelNode>
{
public:
  virtual void attachNode(loka::app::scene::NodeComposition &c)
  {
