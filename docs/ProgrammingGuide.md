...
この見方を先に持っておくと、
`BoundState<T>`、`declareStates()`、`FlowSlot`、`currentBoundary()` の意味が追いやすくなります。
...
Node 固有の local state は、通常 Node の member に `BoundState<T>` を持ち、
...
- 自分の `this->state(...)` 由来の `BoundState<T>` を member に持つ: よい
- 自分の `declareStates()` 由来の `BoundState<T>` を member に持つ: Boundary-scoped state として意図が明確ならよい
...
まず Node member の `BoundState<T>` を `this->state(...)` に登録するのが基本です。
