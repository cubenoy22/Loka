...
そのため Loka は、機能を増やすたびに新しい登場人物を増やす方向を避けます。
中心にあるのは、`Node`、`Boundary`、`State`、`Props`、`Flow`、そして platform projection です。
これらを UI、イベント、非同期処理、ネイティブ反映まで一貫して使い回すことで、
コードの意味と lifecycle を同じ語彙で追えるようにします。

この一貫性は単なる見た目の整理ではありません。
C++98、例外なし、GC なしの環境であっても、
通常のアプリコードからは手動メモリ管理の存在をできるだけ感じさせず、
一方で実際の所有者、解放タイミング、更新経路は曖昧にしないための設計です。
...
|         - PreviewToolbarNode
|
+-- Boundary: FloatingInspector
    owner/tracker: FloatingInspector
    state:
      - visibleSection
      - inspectorWidth
...
つまり、`this->state(...)` は「Node が state を直接 heap に持つ」という意味ではありません。
Node は state の意味とハンドルを持ち、Boundary がその state storage と tracker を支えます。
この分離により、C++98 でも hook 的に書きながら、
実際の lifetime と cleanup は明示的な owner に揃えられます。

小さな例にすると、1 つの Boundary の中に複数 Node-local state が入る形になります。

```text
Boundary: CounterPanel
  owner/tracker
  |
  +-- CounterNode
  |     this->state(count_, 0)
  |     this->state(label_, "Count: 0")
  |
  +-- IncrementButtonNode
  |     reads: count/label through props or borrowed state
  |
  +-- ResetButtonNode
        emits: reset intent
```
...
`NodeState<T>` は、Node が意味的に持つ State ハンドルです。
ただし state storage と tracker は、その Node が attach された Boundary owner に属します。
Boundary も Node の一種なので、Boundary-scoped state にも同じ型を使えます。
...
`this->state(...)` や `declareStates()` のような流れで扱う対象がこれです。

ここで重要なのは、
`this->state(...)` を使うと、
Node member として見える状態が、attach された Boundary の owner/tracker に
自動的に接続されることです。

`declareStates(count)` は、同じ owner path へ複数の Node-local state をまとめて接続する
batch declaration の入口です。大量の `NodeState<T>` を持つ Node では、
state ごとの registration allocation や重複 scan を避けやすくなります。
ただし、普通の少数の Node 固有 state では、
constructor で `this->state(...)` に登録するほうを優先します。

これは単なる便利 API ではありません。
Loka における state ownership と lifecycle を安全に揃えるための、
かなり大切な入口です。

## 5. Boundary-first で考える
...
app / DSL コードで mutable state を扱う通常ルートは、基本的に次の 2 つです。

1. Node-local state backed by the attached Boundary owner
2. repository-owned shared/global state

Node 固有の local state は、通常 Node の member に `NodeState<T>` を持ち、
constructor で `this->state(...)` に登録します。
...
これが通常の owner path です。
Node は値を member として見せますが、実際の state storage と tracking は、
Node が attach された Boundary owner に属します。

`declareStates(count)` は、複数の Node-local state をまとめて宣言する batch API です。
大量の Node-local state を持つ場合や Classic/68k で登録コストを抑えたい場合に使います。
将来的には、Props relay を避けたい shared context 的な Boundary state の宣言にも広げる余地があります。
普通の少数の Node 固有 state では `this->state(...)` を先に検討してください。
...
owner に一致する `NodeState<T>` に対してだけ、意味を持つ path です。

要点は次の通りです。

- owner が自分の `NodeState<T>` を読む/更新する
- child から ancestor-owned `NodeState<T>` を直接 mutate しない
...
特に大事なのは、`NodeState<T>` は cross-boundary transport 用の型ではない、
という点です。

`NodeState<T>` を member として保持してよいのは、
その Node / component 自身が `this->state(...)` または `declareStates()` で登録した
owner-side local state に限る、と考えるのが基本です。

つまり、

- 自分の `this->state(...)` 由来の `NodeState<T>` を member に持つ: よい
- 自分の `declareStates()` 由来の `NodeState<T>` を member に持つ: Boundary-scoped state として意図が明確ならよい
- 親や別 boundary 由来の `NodeState<T>` を props/member で運ぶ: 避ける
...
子から親を変えたい場合でも、
通常は親の `NodeState<T>` を直接 `set()` しません。
...
## 6. `dangerously*` は escape hatch である
...
- `this->state(...)` で Node-local state を owner-side local state として登録する
...
特に raw の `MutableState<T>*` や `NodeState<T>` を props に直接持たせる前提ではありません。
...
- constructor で Node-local state を `this->state(...)` に登録する
- `attachNode()` では event binding など attach 時に必要な接続を行う
- `composeNode()` で UI を組み立てる
- ハンドラ内では `StateTrackerGuard` の下で state を更新する

という役割分担も見ておくと、その後の章が読みやすくなります。

## 10. State で切り替える Toggle UI
...
public:
  ...
  {
    this->state(this->detailsVisible_, false);
  }
...
private:
  ...
  loka::app::scene::NodeState<bool> detailsVisible_;
...
## 11. 同じ UI を Toolbox と macOS に投影する
...
## 12. Flow と async without async/await
...
```cpp
typedef loka::dsl::FlowChain<Input, Output> MyFlow;
...
この形では、`buildFlow()` は pipeline 定義に集中し、
Node 側の `FlowSlot` が trigger / tracker / cleanup を持つ場所になります。
`run()` / `runResult()` / `resumeResult()` / `cancel()` も slot 経由で呼べるため、
長く生きる Flow を裸の pointer として扱う必要がありません。

### Flow と mutable state を混ぜすぎない

Flow は「可変な状態をたくさん共有する場所」ではありません。
むしろ、入力、変換、検証、結果反映を小さな段階に分け、
どの段階が何を読むか、最後にどの owner が state を更新するかを明確にするための仕組みです。

特に、複数の Flow や複数の UI 入力から同じ `MutableState<T>*` を直接共有すると、
どの入力が最後に値を決めたのか、どの変換が再更新を起こしたのか、
どこで止めるべきなのかが追いにくくなります。

基本形は次のように考えます。

```text
Preferred Flow state shape

read-only input state
  -> Flow-local parse / validate / convert
  -> Flow result
  -> owner method applies result to owner state
```
...
Flow A ----+
          +--> shared MutableState<T> --> Flow C
Flow B ----+
```

値が変わる必要があるからといって、
その値をすぐ共有 mutable state にする必要はありません。
...
`onFailure` の列は、すでに「上から matcher を当てて最初に当たった腕だけが走る」
first-match-wins の分岐です。`Match()` はその構造に *値* 用の名前を与え、
腕を「値ハンドラ」または「親が完了を待つ子 Flow」のどちらかにできるようにします。
...
たとえば、メートルとヤード、ポンドとキログラムのような双方向変換では、
片方の入力を変換してもう片方へ反映し、
さらにそれが元の入力へ戻ってくる可能性があります。
浮動小数点や文字列フォーマットが入ると、
意味的には同じ値でも少し違う値として扱われ、
延々と更新されることがあります。

このような UI では、次を意識します。

- どちらの入力が現在の authority なのかを明示する
- 入力中の文字列と、確定済みの数値を分ける
- Slider などの連続入力は、整数 step や fixed-point など比較しやすい値に丸める
- 表示結果が変わらないなら `set()` しない
- 浮動小数点を UI state に戻す場合は、epsilon や quantize の方針を明示する
- `updating_` flag や version counter を app-local に増やす前に、InteractionGroup / Flow gate / owner method のような明示的な形を検討する

将来的には、Loka 側で distinct-until-changed、quantize、throttle、
debounce、stale-result dropping、interaction group、`onUpdateLoopError`
のような標準部品を用意する予定です。
アプリ側では、隠れた mutable object を作るのではなく、
こうした名前のある部品で相互作用を表現する方向を目指します。

重要なのは、Flow をワンショットの stack DSL としてだけ扱わないことです。
UI が生きているあいだ保持され、Node が消えると解放される処理パイプラインとして扱うと、
callback / dialog / file loading のような遅延処理でも ownership を追いやすくなります。

将来的には、非表示 Node の Flow を stall / resume するような lifecycle-aware 制御も検討します。
ただし現時点では、まず「Flow を明示的な owner に保持し、Node の寿命で解放する」ことを基本にします。
...
- 親全体の事実なら親が持つ
- 特定の部分だけのローカル状態ならその Node が `this->state(...)` で登録する
- 子はできるだけ「受け取って読む側」に寄せる

そして、Node ローカルな状態を作るときは、
まず Node member の `NodeState<T>` を `this->state(...)` に登録するのが基本です。
これにより、状態の所属先が曖昧になりにくくなります。
ただし、大量の Node-local state をまとめて宣言したい場合は、
`declareStates(count)` のほうが Classic/68k に優しい選択肢になりえます。
...
この問題を避けるため、
Node ローカルな状態は `this->state(...)` で登録するのが基本です。
...
### Boundary の中に Boundary がある場合

Boundary は入れ子にできます。
このとき、親 Boundary は子 Boundary の内部構造や内部 State を知ろうとしません。
親から見る子 Boundary は、あくまで 1 つの Node です。

子 Boundary の内側で State が更新された場合、その更新は子 Boundary の tracker によって観測され、
子 Boundary 自身の composition / dirty state / platform projection へ流れます。
親 Boundary が子の中身を直接再構成したり、
子の内部 Node を前提にして更新判断をするべきではありません。

これは Loka の重要な分離です。

- 親 Boundary は、子 Boundary を Node として配置する
- 子 Boundary は、自分の State と更新を自分の境界内で扱う
- platform projection は、必要に応じて Boundary 単位の dirty/layout 情報を使う
- 子の中身を覗くのではなく、子 Boundary が外へ出す layout/paint 結果を見る

Boundary は Node の一種ですが、native view や native control と 1:1 に対応するとは限りません。
多くの場合、Boundary は headless な論理境界です。
そのため、メモリ管理や ownership の面では tree 上の Node として扱えますが、
platform 側の実体数や native control 数とは一致しない前提で考えます。

横並びの Boundary で、片方のサイズが変わる場合も原則は同じです。
サイズが変わった子 Boundary は、自分の layout 変更を dirty 情報として外へ出します。
親や platform の layout pass は、その結果を使って兄弟 Node の位置やサイズを再計算します。
親 Boundary が「隣の Boundary の中で何が起きたか」を知る必要はありません。

この契約が崩れると、Boundary が単なる見た目の wrapper になってしまい、
state ownership、更新範囲、platform projection の境界が曖昧になります。
Loka では、Boundary-in-Boundary を成立させるために、
親子間の関係を「内部を読む」ではなく「境界が出す結果を投影する」と考えます。

この性質により、Boundary は単なる subtree wrapper ではなく、
異なる composition / update / projection 方針を同じ Scene の中へ閉じ込めて合成する単位になります。
内部のアルゴリズムが違っても、外側との通信が State / StateTracker / dirty result に揃っていれば、
親子にも兄弟にも配置できます。

つまり Boundary は、
「独立したアプリケーション領域を、共通の State communication model の上で安全に並べる」
ためのコンポーネントでもあります。

## 15. StdComposition と Boundary
...
- `Show` や `ForEach` などで同一モデル内に条件分岐や反復を保てる
- 低コストを優先したい
...
別の composition アルゴリズムは、理論上の対称性のために増やすものではありません。
ゲーム、動画、アニメーション、リアルタイム surface など、
通常の UI と異なる timing / redraw / resource reuse が必要になったときに、
その領域を Boundary の中へ隔離して検討するものです。
通常のアプリ UI で見つかった不具合や不足は、
まず標準 path の契約とテストを強くする方向で扱います。

一方で、68k のような極端に制約の強い環境では、
標準 composition とは別の軽量 Boundary/profile を検討する余地があります。
これは通常のアプリモデルを増やすというより、
同じ ownership / StateTracker / dirty result / lifecycle 契約を保ったまま、
診断情報、汎用性、便利さの一部を削ってメモリ使用量やコードサイズを抑えるためのものです。
現代的で実用的な標準 path と、68k 専用に割り切った path は、
同じ Boundary contract の上に並べられるべきです。
...
### 最小の composition node

標準的な Scene DSL では、`StdCompositionNodeFor<T>` を継承した Node の
`composeNode()` で子 Node を宣言します。
...
`composeNode()` は「子の構造を宣言する場所」です。
初期化や binding は `attachNode()`、解除は `detachNode()` に置くことで、
composition の lifecycle を読みやすく保てます。

### Boundary は composition と state の所有単位

`Boundary` は Node でありながら、composition、state storage、tracker、
dirty/update routing の区切りを持つ所有単位です。

通常のアプリコードでは、明示的に subtree の owner を切りたいときに
`Boundary()` を使います。
...
c.declare(VStack()
          << Boundary<InspectorPanel>());
```

Scene root が Boundary ではない場合は、内部で root boundary に包まれます。
ただし、state ownership や recomposition policy を読みやすくしたい場所では、
アプリ側で Boundary を明示したほうが意図が残ります。

### Fragment と NodeComposition helper

ただ子をまとめたいだけなら、独立した lifecycle を持つ Node を増やすより
`Fragment`、通常は短い alias の `F()` を使います。
...
`F()` は見た目のための container ではありません。
C++ DSL 上で、複数の sibling definition を 1 つの nestable expression として
親へ渡すための adapter です。

この性質により、helper 関数が複数 child を返したい場合にも使えます。

```cpp
loka::app::F toolbarItems()
{
  using namespace loka::app;
  return F()
         << Button("Open")
...
親側から見ると、`toolbarItems()` は `VStack << ...` に差し込める 1 つの
definition ですが、その中には複数の child が含まれます。
これは C++98 の式構文で、配列的・spread 的な DSL 表現を安全に扱うための
小さな構造です。

一時的に active parent を切り替えたいときは `ParentScope` を使います。
これは通常のチェインで書きにくい helper 実装や、低レベルな DSL 拡張で使うためのものです。
...
`CompositionScope` は、その `NodeComposition` を一時的な current composition として
扱いたい helper API のためにあります。通常のアプリコードでは、
まず `composeNode()` に渡された `c` を明示的に使う形を優先してください。

### Definition の所有と arena

`c.declare(...)` に渡された DSL definition は、`NodeComposition` に clone され、
composition arena に所有されます。

これにより、次のような一時オブジェクトを使う DSL が成立します。

```cpp
c.declare(loka::app::VStack()
          << loka::app::Text("Temporary definition is safe"));
```

返された definition pointer を手動で `delete` してはいけません。
所有者は `NodeComposition` です。

### 条件分岐は Show で明示する

条件付きの表示は、まず `Show()` のような明示的な構造で表します。

```cpp
c.declare(loka::app::VStack()
          << (loka::app::Show(this->detailsVisible_.get())
              << loka::app::Text("Details")));
```

この方針は、標準 composition のまま条件分岐を扱うための基本です。
より高度な list reuse や key-based reuse は、別の明示的な DSL として設計する対象です。
...
`Show()` や `ForEach` などで再利用ヒントを明示できる形を優先します。
...
UI text には `loka::core::String` を使います。
...
固定文字列なら `Text("Hello")` のような shorthand を使えます。
内部では Loka の string 型に変換されます。

動的に変わる text は、`State<loka::core::String>*` や
`NodeState<loka::core::String>` として持ち、Node/Platform が live state として
読める形にしてください。固定値まで state に載せる必要はありません。
...
## 21. Rust / React 経験者が気になりやすい ownership と共有
...
- 子だけの local state は、その Node の `this->state(...)`
- 親子で共有する事実は、親へ持ち上げる
...
- その Node/Boundary だけで完結するなら局所 state
...
画像ビューアのようなケースでは、
最初は Window 内の Node ローカル state として画像の取得や表示を始めることがあります。
...
Node member として持ち、constructor で `this->state(...)` に登録するのが自然です。
...
public:
  ...
  SearchPanelNode(const PropsType &p)
      : loka::app::scene::StdCompositionNodeFor<SearchPanelNode>(p),
        query_()
  {
    this->state(this->query_, loka::core::String::Literal(""));
  }
...
private:
  loka::app::scene::NodeState<loka::core::String> query_;
};
```

この形だと、
「この state はこの Node のローカル状態だ」とすぐ分かります。

### 悪い例: Node ローカル state を他 Window へ配る

次のように、ある Node のローカル state を
...
{
public:
  typedef loka::app::scene::StdCompositionPropsFor<PanelNode> PropsType;

  PanelNode(const PropsType &p)
      : loka::app::scene::StdCompositionNodeFor<PanelNode>(p),
        localText_()
  {
    this->state(this->localText_, loka::core::String::Literal("Hello"));
  }
...
private:
  loka::app::scene::NodeState<loka::core::String> localText_;
};
```

この形は、

- local state の owner が PanelNode の attached Boundary なのに
- 利用者がその寿命を超えて参照しやすい
