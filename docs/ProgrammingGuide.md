...
`declareStates()` は Boundary composition 側から同じ owner path へ接続する
入口です。特に、将来 React Context API 的な Boundary-scoped state を扱う
土台として残す価値があります。ただし、普通の Node 固有 state では、
Node の constructor で `this->state(...)` に登録するほうを優先します。
...
`declareStates()` は、Boundary composition の中で明示的に state batch を扱うための
API として残っています。将来的には、Props relay を避けたい shared context 的な
Boundary state の宣言にも使える余地があります。
普通の Node 固有 state では `this->state(...)` を先に検討してください。
...
- Boundary-scoped state や将来の context 的共有では `declareStates()` を検討する
