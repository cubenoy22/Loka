...
`declareStates()` は同じ owner path へ複数 state をまとめて接続する
batch declaration の入口です。将来的には、Node-local state をまとめて登録する
68k-friendly な API や、React Context API 的な Boundary-scoped state を扱う
土台として残す価値があります。ただし、普通の少数の Node 固有 state では、
...
`declareStates()` は、複数 state をまとめて宣言する batch API として残っています。
大量の Node-local state を持つ場合や Classic/68k で登録コストを抑えたい場合、
将来的には Node 側の batch registration として使う余地があります。
また、Props relay を避けたい shared context 的な Boundary state の宣言にも向いています。
...
- 大量の Node-local state、68k-friendly な batch registration、Boundary-scoped state、将来の context 的共有では `declareStates()` を検討する
...
将来的な `declareStates()` batch API のほうが Classic/68k に優しい選択肢になりえます。
