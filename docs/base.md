# Loka プロジェクト概要（2025 年 5 月時点 最新仕様）
...
Loka は C++98 互換・型安全・明示的依存管理を重視した、現代的な宣言的 UI フレームワークです。
...
Loka が重視する価値の一つは、**C++98 制約の上でも modern / tough / safe を成立させること**です。

- **modern**：宣言的 DSL、型安全、責務分離、論理 UI と platform projection の分離
- **tough**：後付けの場当たり修正に頼らず、lifecycle・ownership・transaction 境界を先に整える
- **safe**：例外や広域 shared mutable state に頼りすぎず、状態遷移・所有権・cleanup を明示する

この方針では「とりあえず動かして後で直す」よりも、最初から壊れにくい境界を設計することを優先します。
特に以下を重視します。

- 状態変化は tracker / transaction 境界に集約する
- ownership は owner に閉じ、cleanup 経路を分散させない
- フラグを増やし続ける代わりに、小さな state machine やカプセル化を検討する
- platform ごとの実装差より先に、論理 UI 側の整合性と debuggability を保つ
...
Loka プロジェクトでは、**例外（throw/catch）によるエラー処理は原則禁止**とし、
...
## PlatformContext 必須・AppComposition 設計方針（エラー検出ポリシー）


Loka では、AppComposition の生成時に PlatformContext\*が必須です。


- AppComposition のコンストラクタは PlatformContext\*を必ず受け取り、nullptr は許容しません。
- 実装では `assert(context_ && "AppComposition: PlatformContext* must not be nullptr");` で明示的にチェックします。
...
- AppComposition を生成する際は、必ず有効な PlatformContext\*を渡してください
...
## C++98 における RAII・リソース管理方針（loka::core::ScopedPtr の利用）



Loka では C++98 環境においても delete 漏れや例外時のリソースリークを防ぐため、

独自の RAII スマートポインタ `loka::core::ScopedPtr`（`common/loka/core/util/ScopedPtr.hpp`）を標準で利用します。



- **手動 delete は禁止**し、リソース管理は必ずスコープベースで行う（NSAutoreleasePool 的思想）

- 例外安全・RAII 徹底のため、`loka::core::ScopedPtr<T>` で動的リソースを管理
...
loka::core::ScopedPtr<App> app(platformContext.getApp(...));
...
- 詳細は `common/loka/core/util/ScopedPtr.hpp` を参照
...
- スマートポインタ（loka::core::ScopedPtr など）や明示的な clear()/reset() の活用
